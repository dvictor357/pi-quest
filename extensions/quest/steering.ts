...
export function formatQuestStatus(quest: Quest): string {
  ...
  const doing = quest.tasks.filter(
  ...
  const modeTag =
    quest.planningMode === "approve" ? ` · mode: ${quest.planningMode}` : "";
  const approveTag =
    quest.planningMode === "approve" && !quest.planApproved
      ? ` · ⚠ AWAITING`
      : "";
  const verifyTag = quest.verifyOnComplete ? ` · verify: on` : "";
  const gitTag = quest.gitIntegration?.autoCommit
    ? ` · git: ${quest.commits.length}c`
    : "";
  ...
  if (quest.tasks.length === 0) {
    ...
    if (todo.length > 0) {
      ...
      for (const t of todo) {
        ...
        lines.push(
          `${ICON[t.status]} #${i + 1} ${t.content}  [${t.agent}]${fmtDep(t)}`,
        );
      }
    }
    ...
    if (doing.length > 0) {
      ...
      for (const t of doing) {
        ...
        const timeStr = time
          ? ` ⏱ ${t.status === "verifying" ? "verifying " : ""}${time}`
          : "";
        const vInfo =
          t.status === "verifying"
            ? t.verifyResult
    ...
    if (failed.length > 0 || skipped.length > 0) {
      lines.push(
        ``,
        `━━━ ❌ FAILED / SKIPPED (${failed.length + skipped.length}) ━━━`,
      );
      for (const t of [...failed, ...skipped]) {
        ...
        const info =
          t.status === "failed"
            ...
            : "";
        lines.push(
          `${ICON[t.status]} #${i + 1} ${t.content}  [${t.agent}]${info}`,
        );
      }
    }
  }
