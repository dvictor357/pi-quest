...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      team: Type.Optional(
        Type.String({
          description:
            "Team configuration name (e.g. 'engineering', 'research')",
        }),
      ),
      ...
      verifyOnComplete: Type.Optional(
        Type.Boolean({
          description:
            "Auto-verify completed tasks with a verifier sub-agent (default: true)",
          default: true,
        }),
      ),
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      question: Type.String({
        description:
          "The decision to present to the user. Be clear about the tradeoffs.",
      }),
      ...
      context: Type.Optional(
        Type.String({
          description:
            "Background context to help the user make an informed decision.",
        }),
      ),
    }),
    async execute(_id, params, _signal, _onUpdate, ctx) {
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      tasks: Type.Array(
        Type.Object({
          ...
          context: Type.String({
            description:
              "Focused context/instructions for the sub-agent — keep it lean",
          }),
          dependencies: Type.Optional(
            Type.Array(Type.Number(), {
              description:
                "Indices of tasks that must complete first (0-based)",
            }),
          ),
        }),
      ...
      autoStart: Type.Optional(
        Type.Boolean({
          description:
            "Start auto-pilot immediately after saving (default: true)",
          default: true,
        }),
      ),
    }),
    async execute(_id, params, _signal, _onUpdate, ctx) {
      ...
      if (!quest) {
        return {
          content: [
            { type: "text", text: "No active quest. Use quest_create first." },
          ],
          details: {},
        };
      }
      ...
      const needsApproval =
        quest.planningMode === "approve" && !quest.planApproved;
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      result: Type.Optional(
        Type.String({ description: "Brief summary of what happened" }),
      ),
      verifyOutcome: Type.Optional(
        StringEnum(["PASS", "FAIL"] as const, {
          description:
            "Verification outcome. Use on a 'verifying' task to report PASS or FAIL.",
        }),
      ),
    ...
    async execute(_id, params, _signal, _onUpdate, ctx) {
      ...
      if (params.verifyOutcome) {
        ...
        task.verifyResult =
          `[${params.verifyOutcome}] ${params.verifyEvidence || ""}`.trim();
        task.verified = true;
          ...
          content: [
            {
              ...
              text: [
                ...
                params.verifyEvidence
                  ? `  Last evidence: ${params.verifyEvidence}`
                  : "",
              ].join("\n"),
            },
          ],
      ...
      if (params.status === "done" && quest.verifyOnComplete) {
        ...
        if (hasVerifier) {
          ...
          const verifierAgent =
            team?.members.find(
              (m) => m.agent === "verifier" || m.role === "tester",
            )?.agent ?? "verifier";
          return {
            content: [
              {
        ...
        Type.Array(
          Type.Object({
            ...
            content: Type.Optional(
              Type.String({ description: "New task content" }),
            ),
            agent: Type.Optional(
              Type.String({ description: "New sub-agent type" }),
            ),
            context: Type.Optional(
              Type.String({ description: "New context/instructions" }),
            ),
            dependencies: Type.Optional(
              Type.Array(Type.Number(), {
      ...
      if (params.edits) {
        for (const edit of params.edits) {
          ...
          if (edit.dependencies !== undefined)
            task.dependencies = edit.dependencies;
          editsApplied++;
        }
  ...
  pi.registerTool({
    ...
    async execute(_id, _params, _signal, _onUpdate, ctx) {
      ...
      if (git?.autoPR) {
        ...
        if (git.autoBranch) {
          const branches = [
            ...new Set(quest.commits.map((c) => c.branch).filter(Boolean)),
          ];
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      key: Type.String({
        description:
          'Unique key for this finding (e.g. "api-auth", "best-practice-deployment")',
      }),
      value: Type.String({ description: "The research finding content" }),
      category: Type.Optional(
        Type.String({
          description:
            'Optional category for grouping (e.g. "security", "performance", "api")',
        }),
      ),
    }),
    async execute(_id, params, _signal, _onUpdate, ctx) {
  ...
  pi.registerTool({
    ...
    async execute(_id, params, _signal, _onUpdate, ctx) {
      ...
      if (t.verified) {
        ...
        lines.push(
          ``,
          `**Verification:** 🔍 in progress (retries: ${t.verifyRetries})`,
        );
      }
      if (t.commitHash) {
        lines.push(
  ...
  pi.on("agent_end", async (_event, ctx) => {
    ...
    try {
      ...
      const next = nextPendingTask(quest);
      if (!next) {
        const verifyingTasks = quest.tasks.filter(
        ...
        const allDone = quest.tasks.every(
        ...
        if (allDone && !anyFailed) {
            ...
            quest.commits.length > 0
              ? [
                  ...
                  quest.commits
                    ...
                    .join("\n"),
                  quest.commits.length > 5
                    ? `- ... and ${quest.commits.length - 5} more`
                    : "",
                  git?.autoPR
                    ? [
                        ``,
          ...
          if (ctx.hasUI) {
            ...
            if (action === "Skip all failed") {
              ...
              syncQuestToTodo(quest, ctx.cwd);
              ctx.ui.notify(
                `${failedTasks.length} task(s) skipped. Auto-pilot resuming.`,
                "info",
              );
              return;
            }
          ...
          quest.status = "paused";
          quest.pauseReason =
            "Some tasks failed. Review and decide: retry, skip, or redefine.";
          quest.lastFiredTaskIndex = -1;
          quest.sameTaskCount = 0;
          saveQuest(quest, ctx.cwd);
          questCache = quest;
        ...
        if (quest.sameTaskCount > 2) {
          if (ctx.hasUI) {
            ...
            if (action === "Mark as failed") {
              ...
              syncQuestToTodo(quest, ctx.cwd);
              ctx.ui.notify(
                `Task #${next.index + 1} marked failed.`,
                "warning",
              );
              return;
            }
          ...
          if (ctx.hasUI) {
            ctx.ui.notify(
              `Quest paused: stalled task. /quest resume to continue.`,
              "warning",
            );
          } else {
            autoPilotLocked = true;
    ...
    handler: async (args, ctx) => {
      ...
      switch (sub) {
        case "": {
          ...
          if (!quest) {
            ctx.ui.notify(
              "No active quest. Use /quest create <name>: <goal> to start.",
              "info",
            );
            return;
          }
        ...
        case "create": {
          ...
          if (!name) {
            ctx.ui.notify(
              "Usage: /quest create <name>: <goal description>",
              "error",
            );
            return;
          }
          ...
          if (getQuest(ctx.cwd)?.status === "active") {
            ctx.ui.notify(
              "A quest is already active. /quest pause first.",
              "error",
            );
            return;
          }
        ...
        case "team": {
          ...
          if (subCmd === "list" || !rest) {
            ...
            if (names.length === 0) {
              ctx.ui.notify(
                "No teams found. Built-in teams will be created on next load.",
                "info",
              );
              return;
            }
          ...
          ctx.ui.notify(
            "Usage: /quest team [list|install <git-url>|create]",
            "error",
          );
          return;
        }
        case "start": {
          ...
          if (quest.tasks.length === 0) {
            ctx.ui.notify(
              "No tasks planned. Use quest_plan to add tasks first.",
              "error",
            );
            return;
          }
        ...
        case "pause": {
          ...
          syncQuestToTodo(quest, ctx.cwd);
          ctx.ui.notify(
            `Quest "${quest.name}" paused. /quest resume to continue.`,
            "info",
          );
          return;
        }
        ...
        case "approve": {
          ...
          if (quest.planApproved) {
            ctx.ui.notify(
              "Plan already approved. Quest is in progress.",
              "info",
            );
            return;
          }
          if (quest.tasks.length === 0) {
            ctx.ui.notify(
              "No tasks to approve. Use quest_plan to create a plan first.",
              "error",
            );
            return;
          }
        ...
        case "git": {
          ...
          if (!git) {
            ctx.ui.notify(
              "Git integration not configured for this quest.",
              "info",
            );
            return;
          }
        ...
        case "cancel": {
          ...
          if (quest.status === "active") {
            ctx.ui.notify(
              "Cannot cancel an active quest. /quest pause first.",
              "error",
            );
            return;
          }
