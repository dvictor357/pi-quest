...
import { cwdHash, readJSON, writeJSON } from "./utils";
import { loadProjectMemory } from "./utils";
import { SESSION_META_PATH } from "./constants";
...
export function questTaskToTodo(quest: Quest, task: QuestTask, index: number, previous?: SyncedTodoItem): SyncedTodoItem {
	const now = Date.now();
