...
export function syncConventionsToMemory(quest: Quest, cwd: string): void {
  try {
    ...
    const conventions = Array.isArray(existing.conventions)
  ...
  try {
    ...
    if (raw && raw.version === 1 && Array.isArray(raw.tasks)) {
      raw.tasks = raw.tasks.map((t: any) => ({
        ...
        verifyRetries:
          typeof t.verifyRetries === "number" ? t.verifyRetries : 0,
      ...
      const gi =
        raw.gitIntegration && typeof raw.gitIntegration === "object"
          ? raw.gitIntegration
          : {};
      raw.gitIntegration = {
        ...
        branchPrefix:
          typeof gi.branchPrefix === "string" ? gi.branchPrefix : "quest/",
      };
      ...
      if (typeof raw.lastFiredTaskIndex !== "number")
        raw.lastFiredTaskIndex = -1;
  ...
  try {
    ...
    for (const f of files) {
      try {
        ...
        console.error(
          "[pi-quest] rebuildArchiveIndex/read:",
          e,
        ); /* skip corrupt */
      }
    }
...
export function listArchives(
  ...
  try {
    ...
    if (!existsSync(archiveDir)) return [];
    const indexPath = questArchiveIndexPath(cwd);
    const index = readJSON<{ version: 1; entries: any[] } | null>(
      indexPath,
      null,
    );
